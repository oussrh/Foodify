...
- Short link codes: `Restaurant.code` is six Crockford characters (`lib/restaurant-code.ts`: no I, L, O or U, because they are read as other characters), unique, assigned once at creation (`server/restaurant-code-assign.ts`, which retries against the unique index rather than checking first) and never changed. Every link handed to a device is built from it — `/waiter/K7M2QX`, `/kitchen/orders/K7M2QX` — because a uuid is thirty-six characters to read off a screen and a slug moves when a restaurant is renamed. `restaurantIdFromParam` accepts either form, so an address saved to a tablet before codes existed keeps working; it also folds I→1, O→0, U→V and the case, so a code typed by hand lands.
