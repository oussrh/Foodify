...
- Three roles (`UserRole`, named for people in `lib/roles.ts`): SUPER_ADMIN, RESTAURANT_ADMIN, and KITCHEN — an "Order tablet", a device account made on a restaurant's People tab that signs in at `/kitchen/login` with a password only, never a second factor, and works `/kitchen/<code>` and nothing else. `requireRestaurantAccess` refuses KITCHEN; `requireBoardAccess` is the one guard it passes, so a tablet can never write to a menu. Sessions last a month (`auth.ts`).
...
- `components/waiter/` — the waiter's phone, installable (`/orders/manifest?portal=waiter`). `waiter-tables.tsx` polls the open statuses, which include `READY`: `lib/waiter-floor.ts` reads those as ready to carry and builds one tile per table, ready outranking cooking. The floor's one move is carrying an order out (`requireDeliverAction`, the only board move a waiter may make); the Orders tab lists every order on the floor with that button on the ready ones. A tile that goes ready buzzes the phone — `use-ready-alert.ts`, vibration by default and sound only if the waiter asks, remembered per device, because a bell in a dining room is rude where the same bell on the pass is the point. A table has one bill: an order a waiter sends from a table that already has one is an **addition** (`Order.parentId`, its own kitchen ticket labelled "Addition to #N", so nothing already cooking is disturbed), read back through `GET /api/orders/tab` and ruled by `lib/table-tab.ts` (the restaurant's current service day, from its local 04:00; add by default while a ticket is open or the last was served within 90 minutes); Insights counts a bill once (parents), revenue and waits include additions. A bill is closed by a waiter or manager (`Order.closedAt`) and is then never the current tab; a ticket is cancelled or a dish removed directly while NEW, by a kitchen-confirmed request (`OrderChange` PENDING) while ACCEPTED, and only voided by a manager once READY/DONE, always with a reason; nothing is deleted (`OrderLine.removedQuantity`); merge/move/undo and every change are logged in `OrderChange` (`lib/bill-rules.ts`, `lib/bill-structure.ts`, `server/ticket-changes.ts`, `server/bill-changes.ts`). `waiter-menu.tsx` searches both languages and jumps by category; `waiter-review.tsx` is the read-back before sending, holding the per-dish notes and the one for the whole order. One service worker for every staff app (`public/staff-sw.js`, cache keyed by registration scope; `components/staff/use-staff-pwa.ts` registers it for `/{portal}/orders/`, `/kitchen/` or `/waiter/`); it also shows Web Push (`server/push.ts` sends after the response, `PushSubscription` rows saved by `app/actions/push-actions.ts`, endpoints held to the push services' hosts by `lib/schemas/push.ts`; VAPID env, off when unset). Device settings (keep awake, sound) live in `localStorage` through `components/staff/use-device-setting.ts` and are re-applied on load, a gesture-gated one on the first tap; the "This device" sheet is `components/staff/device-setup/`.
- Short link codes: `Restaurant.code` is six Crockford characters (`lib/restaurant-code.ts`: no I, L, O or U, because they are read as other characters), unique, assigned once at creation (`server/restaurant-code-assign.ts`, which retries against the unique index rather than checking first) and never changed. Every link handed to a device is built from it — `/waiter/K7M2QX`, `/kitchen/K7M2QX` — because a uuid is thirty-six characters to read off a screen and a slug moves when a restaurant is renamed. `restaurantIdFromParam` accepts either form, so an address saved to a tablet before codes existed keeps working; it also folds I→1, O→0, U→V and the case, so a code typed by hand lands.
- Sold out: a dish the kitchen has run out of (`Dish.soldOutUntil`, null is available; `lib/availability.ts`). It stays on the menu, marked, and is not orderable — refused by `POST /api/orders` as well as hidden from the add button, because a cart is built in the guest's browser and can be older than the dish. It returns by itself at the restaurant's next local 04:00 (`Restaurant.timeZone`, an IANA zone set in Settings → General, UTC until set; `lib/time-zone.ts` converts, DST included; not midnight: a kitchen is still open past midnight) with no job running — a read after that moment simply sees an available dish. The devices get a screen of their own (`components/availability/`, `/kitchen/<code>/menu` and `/waiter/<id>/availability`: photo first, one tap, no save button); a manager toggles the same flag from the Stock column of the Dishes tab rather than a tab of its own. `requireServiceStaff` is the guard: managers, order tablets and waiters alike, and it is the only write a device account has over the menu.
- Insights: every restaurant's Insights tab (`/{portal}/restaurants/<id>/insights?grain=day|week|month|year`, `components/insights/`, read by `lib/insights-loader.ts` over the SQL in `lib/insights-queries.ts`) scores the window against the one before it (`lib/insights-scores.ts`, `lib/insights-delta.ts`: counts move by %, rates by points) and charts it: views and AR, orders by guest and waiter, revenue, the three waits (accept, prepare, serve), the funnel, top and overlooked dishes (`lib/insights-dishes.ts`), a weekday × hour heatmap in UTC (`lib/insights-rhythm.ts`) and devices. Charts are hand-drawn SVG in `components/insights/charts/` on the `--chart-1..3` tokens (validated palette; fixed slot order, never cycled), with a legend, a hover tooltip and the bucket table folded underneath as their text alternative. The buckets and the arithmetic are `lib/insights.ts` (a total re-weights an average by the orders behind it; an unfinished order is left out of one, never counted as a zero; revenue is minor units, cancelled orders excluded); the grouping is Postgres `date_trunc` and `EXTRACT` on the restaurant's own clock (`(t AT TIME ZONE 'UTC') AT TIME ZONE tz`), the buckets laid out on the same clock (`wallClock`), and the date filter on the raw UTC column so it keeps its index. A view is recorded when a guest opens a dish and a cart add when a guest adds one (`components/menu/track.ts`, beacons to `/api/dish-views` and `/api/cart-adds`, fire and forget): a waiter's cart is not counted, because a staff order is counted as an order.
...
### POS integration
- One adapter contract (`lib/pos/contract.ts`, `lib/pos/registry.ts`; a Test POS, the vendors listed as coming soon); the owner connects from Settings → Integrations → POS, and so can the admin portal, which has the same restaurant screens (`requireRestaurantAccess`; no super-admin switch: an owner manages everything about their restaurant, and plan limits belong to the future subscription work; `Restaurant.posEnabled` is deprecated and unread). Events go out through `PosOutbox`, written in the same transaction as the ticket, change or close (`server/pos/enqueue.ts`), delivered by `server/pos/deliver.ts` (claimed `FOR UPDATE SKIP LOCKED`, per-bill order, backoff in `lib/pos/outbox-rules.ts`); the POS answers through `/api/pos/webhook/<provider>` into `PosInbox`. Credentials: `server/pos-crypto.ts` (AES-256-GCM, `POS_ENCRYPTION_KEY`). The schema is the `prisma/` folder.
...
- `pnpm test:integration`: `tests/integration/**` on a real Postgres in rolled-back transactions (the ORM is not mocked); `pnpm e2e`: Playwright + axe on the production build, on its own database (`scripts/ci/e2e.mjs`: the `e2e` database of the same Docker container locally, never the one `.env` names: without Docker or `E2E_DATABASE_URL` it refuses).
