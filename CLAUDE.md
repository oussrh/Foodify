...
### POS integration
- One adapter contract (`lib/pos/contract.ts`, `lib/pos/registry.ts`; a Test POS, the vendors listed as coming soon); the owner connects from Settings → Integrations → POS (`requirePosAccess`: restaurant access, then `Restaurant.posEnabled`, which only the super admin sets). Events go out through `PosOutbox`, written in the same transaction as the ticket, change or close (`server/pos/enqueue.ts`), delivered by `server/pos/deliver.ts` (claimed `FOR UPDATE SKIP LOCKED`, per-bill order, backoff in `lib/pos/outbox-rules.ts`); the POS answers through `/api/pos/webhook/<provider>` into `PosInbox`. Credentials: `server/pos-crypto.ts` (AES-256-GCM, `POS_ENCRYPTION_KEY`). The schema is the `prisma/` folder.
