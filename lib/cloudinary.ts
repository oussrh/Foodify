...
// Function for dish images (with dish-specific public IDs)
export async function uploadDishImage(file: File, restaurantSlug: string, dishId: string) {
  const { CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET } =
    process.env;
  ...
  const timestamp = Math.floor(Date.now() / 1000);
  const folder = `restaurants/${restaurantSlug}/dishes`;
  
  // Use dish-specific public ID (will replace existing dish image)
...
// Function for 3D models
export async function uploadDishModel(file: File, restaurantSlug: string, dishId: string) {
  const { CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET } =
    process.env;
  ...
  const timestamp = Math.floor(Date.now() / 1000);
  const folder = `restaurants/${restaurantSlug}/models`;
  
  // Use dish-specific public ID (will replace existing 3D model)
  const publicId = `model_${dishId}`;
