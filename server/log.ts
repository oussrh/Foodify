...
const SECRET_FIELDS = [
  ...
  'DATABASE_URL', 'RESEND_API_KEY', 'CLOUDINARY_API_SECRET', 'NEXTAUTH_SECRET', 'AUTH_SECRET',
]

/**
 * Field paths never written: credentials and second factors, the session, reset and change-email
 * tokens, the request headers that carry them, addresses (personal data in a log is a leak
 * waiting for a breach) and the environment's secrets should a record ever carry them. `code`
