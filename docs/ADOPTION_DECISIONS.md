...
## 2026-09-21 · phase 13 · pino to stdout, no transport; redaction by path, ids instead of addresses; `server/` for the process

- **Situation**: OBS.1 wants a structured logger as the only writer of server output, redaction by field path at the logger, `no-console` at error on the server's paths. The server had thirteen console calls, three printing the sign-in address, two the Cloudinary signing parameters and the signature.
- **Default taken**: pino 10 (`server/log.ts`), JSON lines to stdout through a synchronous destination (`pino.destination({ sync: true })`: the reviewer found the first draft trusting pino's default, which is an asynchronous write flushed by the exit a frozen or killed instance never reaches) and nothing else: no pretty transport (a worker thread, and the dev terminal reads JSON well enough), no file, no shipper (the platform captures stdout and indexes JSON fields). The redaction list names fields wherever they sit (`field` and `*.field`), plus `code` at the top and under `credentials` only: `err.code` (ECONNREFUSED) and an API code are what a log is for. Addresses are redacted, so a call site logs a user id and nothing else identifying; the OTP request logs the portal and the error. The two debug prints of the data a dish action saves and the signer's prints are removed rather than moved: a signature in a log is a leak and the saved row is the evidence. `server/` is opened for the process's modules (the logger, the drain), a leaf like `lib/` and server-only; the standard's layout has such a directory, and the seven server-only `lib` modules may follow later (phase 12's decision). The censor is `<redacted>`, a value rather than an absence, and not a string the measure's hand-masking scan counts.
- **Alternative set aside**: a hand-written JSON logger (sixty lines and its own redaction semantics against pino's tested `fast-redact`); `pino-pretty` in development (a transport, a worker thread and a runtime dependency for a nicer terminal); redacting `*.code` (it would hide every error code); `no-console` on `components/` (the browser's console is the browser's; sixty calls there are a separate pass); an error tracker (OBS-TRACKER, a SHOULD: a vendor, a DSN and a data-flow decision the user makes; recorded as missing on purpose).
- **Re-read when**: a log shipper wants a transport (then `pino.transport` in a worker is the question, and the serverless freeze is the risk), a record needs a field the list would redact (`code` on a payment, say), or a client-side logger is wanted.

## 2026-09-21 · phase 13 · the drain on a Next server: health first, Next's own close, the pool after the grace

- **Situation**: OBS.1 wants SIGTERM to drain the work in flight and never exit on the spot, with a health endpoint the balancer reads that the drain fails first. `next start` already handles SIGTERM, with a listener registered before ours: `server.close()` (the pending requests finish), then the Next server's close, then exit 143; that the platform sends the same signal before retiring a serverless instance is assumed, not verified here. What was missing: nothing failed the health check first, and nothing released the pool should a request outlive Next's close.
- **Default taken**: `server/drain.ts` registers one handler (`instrumentation.ts`, Node runtime only): the draining flag flips at once (`/api/health` answers 503 `draining` before touching the database), a ten-second grace runs on an `unref`'d timer (longer than any request here, shorter than what a platform allows), then `prisma.$disconnect()`, and the exit is left to Next's handler beside ours (an `exit` is injectable for a process without one). On an ordinary stop Next exits before the grace ends and the pool goes with the process: the release runs only when a request outlives ten seconds. `/api/*` is exempt from the host rewrite in `proxy.ts`: an `admin.` or restaurant host sent `/api/health` (and the dish-view beacon) to a 404 under its rewritten prefix. Registration is idempotent and returns its remover, so a hot reload cannot stack handlers and a test can clean up. `/api/health` is `force-dynamic` (a GET that reads no request is prerendered otherwise, and a health baked at build says "ok" for good) and asks `SELECT 1` through the same client the requests use.
- **Alternative set aside**: `NEXT_MANUAL_SIG_HANDLE` and our own server close (instrumentation cannot reach the HTTP server; Next's close is the right one); counting requests in flight ourselves (the proxy sees a request start and never its end); a health without the database (a dead pool would read as alive).
- **Re-read when**: the drill is run on a Linux box (`kill -TERM` under load, watch the health flip and the requests finish: the review ceiling the standard names; Windows has no SIGTERM to send), a background worker appears (then the health must know it), or the grace has to follow a platform's limit.

## 2026-09-21 · phase 13 · one raw environment read outside the env module: the runtime switch the bundler needs

- **Situation**: VALID.3 (`valid.rawEnv`, hard at zero) allows `process.env` in the env module only. `instrumentation.ts` runs `register` in every server runtime Next builds, and its drain (a signal, a pool) is Node-only. Next eliminates the other runtime's branch, and the imports under it, only when it sees `process.env.NEXT_RUNTIME === 'nodejs'` as a literal; read through `publicEnv` the branch stays and the edge bundle carries five warnings about Node APIs.
...
- **Situation**: the phase-11 entry above held JSDoc on the boundary surface and not on components. Measured on 2026-09-23 over the whole tree: 351 exported components and hooks and 53 exported props types under `components/`, and 102 page, layout and route exports under `app/`, with no block. Most component files open with a `//` header saying what they are for, which an IDE does not show where the component is used; the user asked for the components to be held.
- **Default taken** (the user's decision): the same rule, `publicOnly` with the fixer off and `no-types`, on `components/**` one directory at a time, each added to the list in `eslint.config.mjs` in the commit that documents it; the list only grows. First: `components/insights`, `components/orders`, `components/waiter` (40 blocks). Functions only: the contexts are exported function declarations, exported function-valued constants and the default export, **not** props types or interfaces. The phase-11 reason still stands for those — a props type is read field by field, and a block on it restates the props. A component's block says what the component is for (the file's header, made visible where it is used), never what its props are.
...
- **Re-read when**: a directory is added to the list, `app/` pages are taken on, or a block is found restating a signature.
