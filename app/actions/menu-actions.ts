...
export async function createCategory(rawRestaurantId: string, raw: CategoryInput) {
  ...
  return prisma.menuCategory.create({ select: { id: true, nameEn: true, nameFr: true, sortOrder: true, isActive: true },
...
export async function updateCategory(rawId: string, raw: CategoryPatch) {
  ...
  return prisma.menuCategory.update({ select: { id: true, nameEn: true, nameFr: true, sortOrder: true, isActive: true }, where: { id }, data })
}
...
export async function toggleCategoryStatus(rawId: string) {
  ...
  return prisma.menuCategory.update({ select: { id: true, nameEn: true, nameFr: true, sortOrder: true, isActive: true },
    where: { id },
...
export async function deleteCategory(rawId: string) {
  await requireCategoryAccess(rawId)
  const id = uuid.parse(rawId)
  return prisma.menuCategory.delete({ select: { id: true, nameEn: true, nameFr: true, sortOrder: true, isActive: true }, where: { id } })
}
...
export async function createSubcategory(rawCategoryId: string, raw: CategoryInput) {
  ...
  return prisma.menuSubcategory.create({ select: { id: true, nameEn: true, nameFr: true, sortOrder: true, isActive: true },
...
export async function updateSubcategory(rawId: string, raw: CategoryPatch) {
  ...
  const id = uuid.parse(rawId)
  const data = categoryPatch.parse(raw)
  return prisma.menuSubcategory.update({ select: { id: true, nameEn: true, nameFr: true, sortOrder: true, isActive: true }, where: { id }, data })
}
...
export async function toggleSubcategoryStatus(rawId: string) {
  ...
  return prisma.menuSubcategory.update({ select: { id: true, nameEn: true, nameFr: true, sortOrder: true, isActive: true },
    where: { id },
...
export async function deleteSubcategory(rawId: string) {
  await requireSubcategoryAccess(rawId)
  const id = uuid.parse(rawId)
  return prisma.menuSubcategory.delete({ select: { id: true, nameEn: true, nameFr: true, sortOrder: true, isActive: true }, where: { id } })
}
